	...
	public List<Praticien> findByNomLieuSpecialite(String nom, String lieu, String libelle) {
		...
		try {
			...
			if ((nom != null)&&(lieu != null)&&(libelle != null)) {
				query = em.createQuery("select p from praticien p left join fetch p.specialite left join fetch p.lieux where p.nom = :param1 and :param2 member of p.lieux and :param3 member of p.specialites", Praticien.class);
